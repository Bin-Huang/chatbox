    ...
    export default class Base {
        ...
        // Add new method for function calling
        async callChatCompletionWithFunctions(messages: Message[], functions: any[], signal?: AbortSignal, onResultChange?: onResultChange): Promise<string> {
        ...
        async chat(
            ...
            functions?: any[] // Add functions parameter
        ): Promise<string> {
            messages = await this.preprocessMessage(messages)
            return await this._chat(messages, onResultUpdated, functions)
        }
        ...
        protected async _chat(
            ...
            try {
                ...
                // Check if functions are provided and use appropriate method
                if (functions && functions.length > 0) {
                    result = await this.callChatCompletionWithFunctions(
                        messages, 
                        functions,
                        controller.signal, 
                        onResultChange
                    ...
                    result = await this.callChatCompletion(
                        messages, 
                        controller.signal, 
                        onResultChange
                    )
